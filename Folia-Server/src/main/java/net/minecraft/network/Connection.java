...
public class Connection extends SimpleChannelInboundHandler<Packet<?>> {
    ...
    private final Queue<WrappedConsumer> pendingActions = new ca.spottedleaf.concurrentutil.collection.MultiThreadedQueue<>(); // Paper // Folia - region threading - connection fixes
    ...
    private static ChannelInboundHandler createFrameDecoder(@Nullable BandwidthDebugMonitor packetSizeLogger, boolean local) {
        return (ChannelInboundHandler) (!local ? new Varint21FrameDecoder(packetSizeLogger) : (packetSizeLogger != null ? new MonitoredLocalFrameDecoder(packetSizeLogger) : new LocalFrameDecoder()));
    }
