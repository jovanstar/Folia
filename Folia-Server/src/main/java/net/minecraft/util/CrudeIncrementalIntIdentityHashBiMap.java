...
public class CrudeIncrementalIntIdentityHashBiMap<K> implements IdMap<K>, ca.spottedleaf.moonrise.patches.fast_palette.FastPalette<K> { // Paper - optimise palette reads
    ...
    private CrudeIncrementalIntIdentityHashBiMap(K[] values, int[] ids, K[] idToValues, int nextId, int size) {
        this.keys = values;
        this.values = ids;
        this.byId = idToValues;
        this.nextId = nextId;
        this.size = size;
    }
    ...
    public K byId(int index) {
        return index >= 0 && index < this.byId.length ? this.byId[index] : null;
    }
    ...
    private int hash(@Nullable K value) {
        return (Mth.murmurHash3Mixer(System.identityHashCode(value)) & 2147483647) % this.keys.length;
    }
