...
public enum Direction implements StringRepresentable, ca.spottedleaf.moonrise.patches.collisions.util.CollisionDirection { // Paper - optimise collisions
    ...
    private Direction(
        ...
        this.data3d = id;
        this.data2d = horizontalQuarterTurns;
    ...
    public static Direction from2DDataValue(int quarterTurns) {
        return BY_2D_DATA[Mth.abs(quarterTurns % BY_2D_DATA.length)];
    }
    ...
    public static Direction getNearest(int x, int y, int z, @Nullable Direction fallback) {
        int i = Math.abs(x);
        int j = Math.abs(y);
        int k = Math.abs(z);
        if (i > k && i > j) {
    ...
    public static Direction getNearest(Vec3i vec, @Nullable Direction fallback) {
        return getNearest(vec.getX(), vec.getY(), vec.getZ(), fallback);
    }
