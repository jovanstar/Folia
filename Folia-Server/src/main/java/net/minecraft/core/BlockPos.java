...
public class BlockPos extends Vec3i {
    public static final Codec<BlockPos> CODEC = Codec.INT_STREAM
        .<BlockPos>comapFlatMap(
            stream -> Util.fixedSize(stream, 3).map(values -> new BlockPos(values[0], values[1], values[2])),
    ...
    public BlockPos relative(Direction direction, int i) {
        return i == 0
            ? this
            : new BlockPos(this.getX() + direction.getStepX() * i, this.getY() + direction.getStepY() * i, this.getZ() + direction.getStepZ() * i);
    }
    ...
    public static int breadthFirstTraversal(
        ...
        Function<BlockPos, BlockPos.TraversalNodeStatus> callback
    ) {
        ...
        while (!queue.isEmpty()) {
            ...
            if (longSet.add(l)) {
                BlockPos.TraversalNodeStatus traversalNodeStatus = callback.apply(blockPos);
