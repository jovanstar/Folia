...
public class PlaceCommand {
    ...
    public static int placeFeature(CommandSourceStack source, Holder.Reference<ConfiguredFeature<?, ?>> feature, BlockPos pos) throws CommandSyntaxException {
        ...
        // Folia start - region threading
        worldserver.moonrise$loadChunksAsync(
    ...
    public static int placeStructure(CommandSourceStack source, Holder.Reference<Structure> structure, BlockPos pos) throws CommandSyntaxException {
        ...
        worldserver.moonrise$loadChunksAsync(
            ...
            (chunks) -> {
                try {
                    // Folia end - region threading
        StructureStart structurestart = structure1.generate(structure, worldserver.dimension(), source.registryAccess(), chunkgenerator, chunkgenerator.getBiomeSource(), worldserver.getChunkSource().randomState(), worldserver.getStructureManager(), worldserver.getSeed(), new ChunkPos(pos), 0, worldserver, (holder) -> {
            return true;
        });
        ...
        if (!structurestart.isValid()) {
            ...
            ChunkPos.rangeClosed(chunkcoordintpair, chunkcoordintpair1).forEach((chunkcoordintpair2) -> {
                structurestart.placeInChunk(worldserver, worldserver.structureManager(), chunkgenerator, worldserver.getRandom(), new BoundingBox(chunkcoordintpair2.getMinBlockX(), worldserver.getMinY(), chunkcoordintpair2.getMinBlockZ(), chunkcoordintpair2.getMaxBlockX(), worldserver.getMaxY() + 1, chunkcoordintpair2.getMaxBlockZ()), chunkcoordintpair2);
            });
