...
public class SpreadPlayersCommand {
        ...
        for (Iterator iterator = entities.iterator(); iterator.hasNext(); d0 += d1) {
            ...
            if (respectTeams) {
                ...
                if (!map.containsKey(scoreboardteam)) {
                    map.put(scoreboardteam, piles[j++]);
                }
    ...
    private static class Position {
        ...
        public int getSpawnY(BlockGetter blockView, int maxY) {
            ...
            for (boolean flag2 = blockView.getBlockState(blockposition_mutableblockposition).isAir(); blockposition_mutableblockposition.getY() > blockView.getMinY(); flag2 = flag1) {
                blockposition_mutableblockposition.move(Direction.DOWN);
