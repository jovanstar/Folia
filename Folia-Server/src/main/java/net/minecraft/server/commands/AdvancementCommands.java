...
public class AdvancementCommands {
    ...
    private static final Dynamic2CommandExceptionType ERROR_CRITERION_NOT_FOUND = new Dynamic2CommandExceptionType(
        (advancement, criterion) -> Component.translatableEscape("commands.advancement.criterionNotFound", advancement, criterion)
    );
                                                    ...
                                                    Commands.argument("criterion", StringArgumentType.greedyString())
                                                        .suggests(
                                                            (context, builder) -> SharedSuggestionProvider.suggest(
                                                                    ResourceKeyArgument.getAdvancement(context, "advancement").value().criteria().keySet(),
                                                                    builder
                                                                )
                                                        )
                                                        .executes(
    ...
    private static int perform(
        CommandSourceStack source, Collection<ServerPlayer> targets, AdvancementCommands.Action operation, Collection<AdvancementHolder> selection
        ...
        for (ServerPlayer serverPlayer : targets) {
            ...
            serverPlayer.getBukkitEntity().taskScheduler.schedule((ServerPlayer player) -> {
                operation.perform(player, selection);
            }, null, 1L);
                ...
                if (targets.size() == 1) {
                    throw ERROR_NO_ACTION_PERFORMED.create(
                        Component.translatable(
                            ...
                            Advancement.name(selection.iterator().next()),
                            targets.iterator().next().getDisplayName()
                        )
                    );
                } else {
                    throw ERROR_NO_ACTION_PERFORMED.create(
                        Component.translatable(operation.getKey() + ".one.to.many.failure", Advancement.name(selection.iterator().next()), targets.size())
                    );
                }
            } else if (targets.size() == 1) {
                throw ERROR_NO_ACTION_PERFORMED.create(
                    Component.translatable(operation.getKey() + ".many.to.one.failure", selection.size(), targets.iterator().next().getDisplayName())
                );
            } else {
                throw ERROR_NO_ACTION_PERFORMED.create(Component.translatable(operation.getKey() + ".many.to.many.failure", selection.size(), targets.size()));
            }
        } else {
            if (selection.size() == 1) {
                if (targets.size() == 1) {
                    ...
                    source.sendSuccess(
                        () -> Component.translatable(operation.getKey() + ".one.to.many.success", Advancement.name(selection.iterator().next()), targets.size()),
                        true
                    );
                }
            } else if (targets.size() == 1) {
                source.sendSuccess(
                    () -> Component.translatable(operation.getKey() + ".many.to.one.success", selection.size(), targets.iterator().next().getDisplayName()),
                    true
                );
            } else {
    ...
    private static int performCriterion(
        CommandSourceStack source, Collection<ServerPlayer> targets, AdvancementCommands.Action operation, AdvancementHolder advancement, String criterion
    ) throws CommandSyntaxException {
        int i = 0;
        ...
        if (!advancement2.criteria().containsKey(criterion)) {
            ...
            for (ServerPlayer serverPlayer : targets) {
                ...
                ++i;
                serverPlayer.getBukkitEntity().taskScheduler.schedule((ServerPlayer player) -> {
                    operation.performCriterion(player, advancement, criterion);
                }, null, 1L);
                // Folia end - region threading
            }
            ...
            if (i == 0) {
                if (targets.size() == 1) {
                    ...
                    throw ERROR_NO_ACTION_PERFORMED.create(
                        Component.translatable(operation.getKey() + ".criterion.to.many.failure", criterion, Advancement.name(advancement), targets.size())
                    );
                }
            } else {
                if (targets.size() == 1) {
                    ...
                    source.sendSuccess(
                        () -> Component.translatable(
                                operation.getKey() + ".criterion.to.many.success", criterion, Advancement.name(advancement), targets.size()
                            ),
                        true
                    );
                }
    ...
    private static List<AdvancementHolder> getAdvancements(
        CommandContext<CommandSourceStack> context, AdvancementHolder advancement, AdvancementCommands.Mode selection
    ) {
    ...
    static enum Mode {
        ...
        private Mode(final boolean before, final boolean after) {
            this.parents = before;
            this.children = after;
        }
    }
