...
public class Allay extends PathfinderMob implements InventoryCarrier, VibrationSystem {
    ...
    public static AttributeSupplier.Builder createAttributes() {
        return Mob.createMobAttributes().add(Attributes.MAX_HEALTH, 20.0D).add(Attributes.FLYING_SPEED, 0.10000000149011612D).add(Attributes.MOVEMENT_SPEED, 0.10000000149011612D).add(Attributes.ATTACK_DAMAGE, 2.0D);
    }
    ...
    public boolean hurtServer(ServerLevel world, DamageSource source, float amount) {
        return this.isLikedPlayer(source.getEntity()) ? false : super.hurtServer(world, source, amount);
    }
    ...
    private boolean isLikedPlayer(@Nullable Entity player) {
        if (!(player instanceof Player entityhuman)) {
            return false;
        } else {
    ...
    public boolean wantsToPickUp(ServerLevel world, ItemStack stack) {
        ...
        return !itemstack1.isEmpty() && world.getGameRules().getBoolean(GameRules.RULE_MOBGRIEFING) && this.inventory.canAddItem(stack) && this.allayConsidersItemEqual(itemstack1, stack);
    }
    ...
    protected void pickUpItem(ServerLevel world, ItemEntity itemEntity) {
        InventoryCarrier.pickUpItem(world, this, this, itemEntity);
    }
    ...
    public float getHoldingItemAnimationProgress(float tickDelta) {
        return Mth.lerp(tickDelta, this.holdingItemAnimationTicks0, this.holdingItemAnimationTicks) / 5.0F;
    }
    ...
    public float getSpinningProgress(float tickDelta) {
        return Mth.lerp(tickDelta, this.spinningAnimationTicks0, this.spinningAnimationTicks) / 15.0F;
    }
        ...
        public boolean canReceiveVibration(ServerLevel world, BlockPos pos, Holder<GameEvent> event, GameEvent.Context emitter) {
            if (Allay.this.isNoAi()) {
                ...
                if (optional.isEmpty()) {
                    ...
                    return globalpos.isCloseEnough(world.dimension(), Allay.this.blockPosition(), 1024) && globalpos.pos().equals(pos);
                }
            }
        }
