...
public class Ravager extends Raider {
    ...
    private void roar() {
        if (this.isAlive()) {
            ...
            if (world instanceof ServerLevel) {
                ServerLevel worldserver = (ServerLevel) world;
                Predicate<Entity> predicate = worldserver.getGameRules().getBoolean(GameRules.RULE_MOBGRIEFING) ? Ravager.ROAR_TARGET_WITH_GRIEFING : Ravager.ROAR_TARGET_WITHOUT_GRIEFING;
                List<? extends LivingEntity> list = this.level().getEntitiesOfClass(LivingEntity.class, this.getBoundingBox().inflate(4.0D), predicate);
                ...
                List<? extends LivingEntity> list1 = this.level().getEntitiesOfClass(LivingEntity.class, this.getBoundingBox().inflate(4.0D), Ravager.ROAR_TARGET_ON_CLIENT);
                Iterator iterator1 = list1.iterator();

                while (iterator1.hasNext()) {
                    LivingEntity entityliving1 = (LivingEntity) iterator1.next();

                    this.strongKnockback(entityliving1);
                }
