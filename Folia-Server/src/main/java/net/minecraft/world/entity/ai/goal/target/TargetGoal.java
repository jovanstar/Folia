...
public abstract class TargetGoal extends Goal {
    ...
    public boolean canContinueToUse() {
        ...
        if (entityliving == null) {
            ...
            PlayerTeam scoreboardteam = this.mob.getTeam();
            PlayerTeam scoreboardteam1 = entityliving.getTeam();
    ...
    protected boolean canAttack(@Nullable LivingEntity target, TargetingConditions targetPredicate) {
        if (target == null) {
            return false;
        } else if (!targetPredicate.test(getServerLevel((Entity) this.mob), this.mob, target)) {
            return false;
