...
public class SkeletonTrapGoal extends Goal {
    ...
    public void tick() {
        ...
        this.horse.setAge(0);
        LightningBolt entitylightning = (LightningBolt) EntityType.LIGHTNING_BOLT.create(worldserver, EntitySpawnReason.TRIGGERED);
    ...
    private AbstractHorse createHorse(DifficultyInstance localDifficulty) {
        SkeletonHorse entityhorseskeleton = (SkeletonHorse) EntityType.SKELETON_HORSE.create(this.horse.level(), EntitySpawnReason.TRIGGERED);

        if (entityhorseskeleton != null) {
            entityhorseskeleton.finalizeSpawn((ServerLevel) this.horse.level(), localDifficulty, EntitySpawnReason.TRIGGERED, (SpawnGroupData) null);
    ...
    private Skeleton createSkeleton(DifficultyInstance localDifficulty, AbstractHorse vehicle) {
        Skeleton entityskeleton = (Skeleton) EntityType.SKELETON.create(vehicle.level(), EntitySpawnReason.TRIGGERED);

        if (entityskeleton != null) {
            entityskeleton.finalizeSpawn((ServerLevel) vehicle.level(), localDifficulty, EntitySpawnReason.TRIGGERED, (SpawnGroupData) null);
