...
public class RamTarget extends Behavior<Goat> {
    ...
    public RamTarget(
        Function<Goat, UniformInt> cooldownRangeFactory,
        TargetingConditions targetPredicate,
        float speed,
        ToDoubleFunction<Goat> strengthMultiplierFactory,
        Function<Goat, SoundEvent> impactSoundFactory,
        Function<Goat, SoundEvent> hornBreakSoundFactory
    ) {
    ...
    protected void tick(ServerLevel world, Goat entity, long time) {
        ...
        if (!list.isEmpty()) {
            ...
            DamageSource damageSource = world.damageSources().noAggroMobAttack(entity);
            if (livingEntity.hurtServer(world, damageSource, (float)entity.getAttributeValue(Attributes.ATTACK_DAMAGE))) {
