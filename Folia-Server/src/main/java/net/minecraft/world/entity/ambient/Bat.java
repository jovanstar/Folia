...
public class Bat extends AmbientCreature {
    ...
    protected void defineSynchedData(SynchedEntityData.Builder builder) {
        ...
        builder.define(Bat.DATA_ID_FLAGS, (byte) 0);
    }
    ...
    protected void customServerAiStep(ServerLevel world) {
        ...
        if (this.isResting()) {
            ...
            if (world.getBlockState(blockposition1).isRedstoneConductor(world, blockposition)) {
            ...
            if (this.targetPosition != null && (!world.isEmptyBlock(this.targetPosition) || this.targetPosition.getY() <= world.getMinY())) {
    ...
    public static boolean checkBatSpawnRules(EntityType<Bat> type, LevelAccessor world, EntitySpawnReason spawnReason, BlockPos pos, RandomSource random) {
        if (pos.getY() >= world.getHeightmapPos(Heightmap.Types.WORLD_SURFACE, pos).getY()) {
            return false;
        } else {
            ...
            return i > random.nextInt(b0) ? false : (!world.getBlockState(pos.below()).is(BlockTags.BATS_SPAWNABLE_ON) ? false : checkMobSpawnRules(type, world, spawnReason, pos, random));
        }
    }
