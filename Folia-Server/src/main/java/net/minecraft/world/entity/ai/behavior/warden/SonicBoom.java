...
public class SonicBoom extends Behavior<Warden> {
    ...
    protected boolean checkExtraStartConditions(ServerLevel world, Warden entity) {
        return entity.closerThan(entity.getBrain().getMemory(MemoryModuleType.ATTACK_TARGET).get(), 15.0, 20.0);
    }
    ...
    protected void tick(ServerLevel world, Warden entity, long time) {
        ...
        if (!entity.getBrain().hasMemoryValue(MemoryModuleType.SONIC_BOOM_SOUND_DELAY)
            ...
            entity.getBrain()
                ...
                .filter(entity::canTargetEntity)
                .filter(target -> entity.closerThan(target, 15.0, 20.0))
                .ifPresent(target -> {
                    Vec3 vec3 = entity.position().add(entity.getAttachments().get(EntityAttachment.WARDEN_CHEST, 0, entity.getYRot()));
                    ...
                    for (int j = 1; j < i; j++) {
                        ...
                        world.sendParticles(ParticleTypes.SONIC_BOOM, vec34.x, vec34.y, vec34.z, 1, 0.0, 0.0, 0.0, 0.0);
                    }
