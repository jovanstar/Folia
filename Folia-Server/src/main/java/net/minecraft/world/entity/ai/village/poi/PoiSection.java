...
public class PoiSection implements ca.spottedleaf.moonrise.patches.chunk_system.level.poi.ChunkSystemPoiSection { // Paper - rewrite chunk system
    ...
    public void refresh(Consumer<BiConsumer<BlockPos, Holder<PoiType>>> updater) {
        if (!this.isValid) {
            ...
            updater.accept((pos, poiEntry) -> {
                ...
                PoiRecord poiRecord = short2ObjectMap.computeIfAbsent(s, sx -> new PoiRecord(pos, poiEntry, this.setDirty));
                this.add(poiRecord);
            });
            this.isValid = true;
            this.setDirty.run();
    ...
    public static record Packed(boolean isValid, List<PoiRecord.Packed> records) {
        ...
        public PoiSection unpack(Runnable updateListener) {
            return new PoiSection(updateListener, this.isValid, this.records.stream().map(packed -> packed.unpack(updateListener)).toList());
        }
    }
}
