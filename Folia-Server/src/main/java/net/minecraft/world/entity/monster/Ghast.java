...
public class Ghast extends FlyingMob implements Enemy {
    ...
    public boolean isInvulnerableTo(ServerLevel world, DamageSource source) {
        return this.isInvulnerable() && !source.is(DamageTypeTags.BYPASSES_INVULNERABILITY) || !Ghast.isReflectedFireball(source) && super.isInvulnerableTo(world, source);
    }

    @Override
    public boolean hurtServer(ServerLevel world, DamageSource source, float amount) {
        if (Ghast.isReflectedFireball(source)) {
            super.hurtServer(world, source, 1000.0F);
            return true;
        } else {
            return this.isInvulnerableTo(world, source) ? false : super.hurtServer(world, source, amount);
        }
    }
    ...
    public static boolean checkGhastSpawnRules(EntityType<Ghast> type, LevelAccessor world, EntitySpawnReason spawnReason, BlockPos pos, RandomSource random) {
