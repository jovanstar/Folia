...
public class Vex extends Monster implements TraceableEntity {
        ...
        public void tick() {
            ...
            if (entityliving != null) {
                if (Vex.this.getBoundingBox().intersects(entityliving.getBoundingBox())) {
                    Vex.this.doHurtTarget(getServerLevel(Vex.this.level()), entityliving);
                    Vex.this.setIsCharging(false);
                } else {
    ...
    private class VexRandomMoveGoal extends Goal {
        ...
        public void tick() {
            ...
            if (blockposition == null || !ca.spottedleaf.moonrise.common.util.TickThread.isTickThreadFor((net.minecraft.server.level.ServerLevel)Vex.this.level(), blockposition)) { // Folia - region threading
