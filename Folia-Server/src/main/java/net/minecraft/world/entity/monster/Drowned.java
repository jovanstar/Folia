...
public class Drowned extends Zombie implements RangedAttackMob {
    ...
    public static boolean checkDrownedSpawnRules(
        EntityType<Drowned> type, ServerLevelAccessor world, EntitySpawnReason spawnReason, BlockPos pos, RandomSource random
    ) {
    ...
    protected boolean canReplaceCurrentItem(ItemStack newStack, ItemStack currentStack, EquipmentSlot slot) {
        return !currentStack.is(Items.NAUTILUS_SHELL) && super.canReplaceCurrentItem(newStack, currentStack, slot);
    }
    ...
    public void performRangedAttack(LivingEntity target, float pullProgress) {
        ...
        if (this.level() instanceof ServerLevel serverLevel) {
            Projectile.spawnProjectileUsingShoot(
                thrownTrident, serverLevel, itemStack2, d, e + g * 0.2F, f, 1.6F, (float)(14 - this.level().getDifficulty().getId() * 4)
            );
        }
