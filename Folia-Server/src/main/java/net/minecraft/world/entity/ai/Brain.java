...
public class Brain<E extends LivingEntity> {
            ...
            (new MapCodec<Brain<E>>() {
                    ...
                    private <T, U> DataResult<Brain.MemoryValue<U>> captureRead(MemoryModuleType<U> memoryType, DynamicOps<T> ops, T value) {
                        return memoryType.getCodec()
                            ...
                            .<ExpirableValue<U>>flatMap(codec -> codec.parse(ops, value))
    ...
    public Brain(
        ...
        ImmutableList<Brain.MemoryValue<?>> memoryEntries,
    ...
    public <U> boolean isMemoryValue(MemoryModuleType<U> type, U value) {
        return this.hasMemoryValue(type) && this.getMemory(type).filter(memoryValue -> memoryValue.equals(value)).isPresent();
    }
    ...
    public boolean checkMemory(MemoryModuleType<?> type, MemoryStatus state) {
        ...
        return optional != null
            && (
                state == MemoryStatus.REGISTERED
                    || state == MemoryStatus.VALUE_PRESENT && optional.isPresent()
                    || state == MemoryStatus.VALUE_ABSENT && optional.isEmpty()
            );
    }
    ...
    public void addActivityAndRemoveMemoryWhenStopped(
        Activity activity, int begin, ImmutableList<? extends BehaviorControl<? super E>> tasks, MemoryModuleType<?> memoryType
    ) {
    ...
    public void addActivityAndRemoveMemoriesWhenStopped(
        ...
        for (Pair<Integer, ? extends BehaviorControl<? super E>> pair : indexedTasks) {
            this.availableBehaviorsByPriority
                .computeIfAbsent(pair.getFirst(), index -> Maps.newHashMap())
    ...
    ImmutableList<? extends Pair<Integer, ? extends BehaviorControl<? super E>>> createPriorityPairs(
        int begin, ImmutableList<? extends BehaviorControl<? super E>> tasks
    ) {
        int i = begin;
                    ...
                    codec -> this.value
                            .ifPresent(
                                data -> builder.add(
                                        ...
                                        codec.encodeStart(ops, (ExpirableValue<U>)data)
                                    )
                            )
                );
        }
