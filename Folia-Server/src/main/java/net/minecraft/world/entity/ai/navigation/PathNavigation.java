...
public abstract class PathNavigation {
    ...
    public PathNavigation(Mob entity, Level world) {
        this.mob = entity;
        this.level = world;
        this.pathFinder = this.createPathFinder(Mth.floor(entity.getAttributeBaseValue(Attributes.FOLLOW_RANGE) * 16.0));
    }
    ...
    public void setRequiredPathLength(float maxFollowRange) {
        this.requiredPathLength = maxFollowRange;
    ...
    protected Path createPath(Set<BlockPos> positions, int range, boolean useHeadPos, int distance) {
        return this.createPath(positions, range, useHeadPos, distance, this.getMaxPathLength());
    }
    ...
    public boolean canCutCorner(PathType nodeType) {
        return nodeType != PathType.DANGER_FIRE && nodeType != PathType.DANGER_OTHER && nodeType != PathType.WALKABLE_DOOR;
    }
    ...
    protected static boolean isClearForMovementBetween(Mob entity, Vec3 startPos, Vec3 entityPos, boolean includeFluids) {
        ...
        return entity.level()
                .clip(new ClipContext(startPos, vec3, ClipContext.Block.COLLIDER, includeFluids ? ClipContext.Fluid.ANY : ClipContext.Fluid.NONE, entity))
                .getType()
            == HitResult.Type.MISS;
    }
