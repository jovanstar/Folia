...
public class PiglinAi {
    ...
    private static void initCoreActivity(Brain<Piglin> brain) {
        brain.addActivity(Activity.CORE, 0, ImmutableList.of(new LookAtTargetSink(45, 90), new MoveToTargetSink(), InteractWithDoor.create(), PiglinAi.babyAvoidNemesis(), PiglinAi.avoidZombified(), StopHoldingItemIfNoLongerAdmiring.create(), StartAdmiringItemIfSeen.create(119), StartCelebratingIfTargetDead.create(300, PiglinAi::wantsToDance), StopBeingAngryIfTargetDead.create()));
    }

    private static void initIdleActivity(Brain<Piglin> brain) {
        brain.addActivity(Activity.IDLE, 10, ImmutableList.of(SetEntityLookTarget.create(PiglinAi::isPlayerHoldingLovedItem, 14.0F), StartAttacking.create((worldserver, entitypiglin) -> {
            return entitypiglin.isAdult();
        }, PiglinAi::findNearestValidAttackTarget), BehaviorBuilder.triggerIf(Piglin::canHunt, StartHuntingHoglin.create()), PiglinAi.avoidRepellent(), PiglinAi.babySometimesRideBabyHoglin(), PiglinAi.createIdleLookBehaviors(), PiglinAi.createIdleMovementBehaviors(), SetLookAndInteract.create(EntityType.PLAYER, 4)));
    }

    private static void initFightActivity(Piglin piglin, Brain<Piglin> brain) {
        brain.addActivityAndRemoveMemoryWhenStopped(Activity.FIGHT, 10, ImmutableList.of(StopAttackingIfTargetInvalid.create((worldserver, entityliving) -> {
            return !PiglinAi.isNearestValidAttackTarget(worldserver, piglin, entityliving);
    ...
    private static void initCelebrateActivity(Brain<Piglin> brain) {
        brain.addActivityAndRemoveMemoryWhenStopped(Activity.CELEBRATE, 10, ImmutableList.of(PiglinAi.avoidRepellent(), SetEntityLookTarget.create(PiglinAi::isPlayerHoldingLovedItem, 14.0F), StartAttacking.create((worldserver, entitypiglin) -> {
            return entitypiglin.isAdult();
    ...
    protected static void pickUpItem(ServerLevel world, Piglin piglin, ItemEntity itemEntity) {
    ...
    protected static void stopHoldingOffHandItem(ServerLevel world, Piglin piglin, boolean barter) {
        ...
        if (piglin.isAdult()) {
            ...
            flag1 = !piglin.equipItemIfPossible(world, itemstack).isEmpty();
            if (!flag1) {
    ...
    private static boolean wantsToStopRiding(Piglin piglin, Entity ridden) {
        if (!(ridden instanceof Mob entityinsentient)) {
            return false;
        } else {
    ...
    private static boolean isNearestValidAttackTarget(ServerLevel world, Piglin piglin, LivingEntity target) {
        return PiglinAi.findNearestValidAttackTarget(world, piglin).filter((entityliving1) -> {
    ...
    private static Optional<? extends LivingEntity> findNearestValidAttackTarget(ServerLevel world, Piglin piglin) {
        Brain<Piglin> behaviorcontroller = piglin.getBrain();
        ...
        if (PiglinAi.isNearZombified(piglin)) {
            ...
            if (optional.isPresent() && Sensor.isEntityAttackableIgnoringLineOfSight(world, piglin, (LivingEntity) optional.get())) {
                return optional;
            } else {
                Optional optional1;
                ...
                if (optional1.isPresent()) {
                    ...
                    return optional2.isPresent() && Sensor.isEntityAttackable(world, piglin, (LivingEntity) optional2.get()) ? optional2 : Optional.empty();
                }
            }
        }
    }
    ...
    public static void angerNearbyPiglins(ServerLevel world, Player player, boolean blockOpen) {
        ...
        list.stream().filter(PiglinAi::isIdle).filter((entitypiglin) -> {
            ...
            if (world.getGameRules().getBoolean(GameRules.RULE_UNIVERSAL_ANGER)) {
                PiglinAi.setAngerTargetToNearestTargetablePlayerIfFound(world, entitypiglin, player);
            } else {
                PiglinAi.setAngerTarget(world, entitypiglin, player);
            }

        });
    }

    public static InteractionResult mobInteract(ServerLevel world, Piglin piglin, Player player, InteractionHand hand) {
        ItemStack itemstack = player.getItemInHand(hand);

        if (PiglinAi.canAdmire(piglin, itemstack)) {
            ItemStack itemstack1 = itemstack.consumeAndReturn(1, player);

            PiglinAi.holdInOffhand(world, piglin, itemstack1);
            PiglinAi.admireGoldItem(piglin);
            PiglinAi.stopWalking(piglin);
    ...
    protected static void wasHurtBy(ServerLevel world, Piglin piglin, LivingEntity attacker) {
        if (!(attacker instanceof Piglin)) {
            if (PiglinAi.isHoldingItemInOffHand(piglin)) {
                PiglinAi.stopHoldingOffHandItem(world, piglin, false);
            }
            ...
            if (piglin.isBaby()) {
                ...
                if (Sensor.isEntityAttackableIgnoringLineOfSight(world, piglin, attacker)) {
                    PiglinAi.broadcastAngerTarget(world, piglin, attacker);
                }
                ...
                PiglinAi.maybeRetaliate(world, piglin, attacker);
            }
        }
    }

    protected static void maybeRetaliate(ServerLevel world, AbstractPiglin piglin, LivingEntity target) {
        if (!piglin.getBrain().isActive(Activity.AVOID)) {
            if (Sensor.isEntityAttackableIgnoringLineOfSight(world, piglin, target)) {
                if (!BehaviorUtils.isOtherTargetMuchFurtherAwayThanCurrentAttackTarget(piglin, target, 4.0D)) {
                    if (target.getType() == EntityType.PLAYER && world.getGameRules().getBoolean(GameRules.RULE_UNIVERSAL_ANGER)) {
                        PiglinAi.setAngerTargetToNearestTargetablePlayerIfFound(world, piglin, target);
                        PiglinAi.broadcastUniversalAnger(world, piglin);
                    } else {
                        PiglinAi.setAngerTarget(world, piglin, target);
                        PiglinAi.broadcastAngerTarget(world, piglin, target);
                    }
    ...
    public static boolean isWearingSafeArmor(LivingEntity entity) {
        Iterator iterator = entity.getArmorAndBodyArmorSlots().iterator();
    ...
    protected static void broadcastAngerTarget(ServerLevel world, AbstractPiglin piglin, LivingEntity target) {
        PiglinAi.getAdultPiglins(piglin).forEach((entitypiglinabstract1) -> {
            if (target.getType() != EntityType.HOGLIN || entitypiglinabstract1.canHunt() && ((Hoglin) target).canBeHunted()) {
                PiglinAi.setAngerTargetIfCloserThanCurrent(world, entitypiglinabstract1, target);
            }
        });
    }

    protected static void broadcastUniversalAnger(ServerLevel world, AbstractPiglin piglin) {
        PiglinAi.getAdultPiglins(piglin).forEach((entitypiglinabstract1) -> {
            PiglinAi.getNearestVisibleTargetablePlayer(entitypiglinabstract1).ifPresent((entityhuman) -> {
                PiglinAi.setAngerTarget(world, entitypiglinabstract1, entityhuman);
            });
        });
    }

    protected static void setAngerTarget(ServerLevel world, AbstractPiglin piglin, LivingEntity target) {
        if (Sensor.isEntityAttackableIgnoringLineOfSight(world, piglin, target)) {
    ...
    private static void setAngerTargetToNearestTargetablePlayerIfFound(ServerLevel world, AbstractPiglin piglin, LivingEntity target) {
        Optional<Player> optional = PiglinAi.getNearestVisibleTargetablePlayer(piglin);

        if (optional.isPresent()) {
            PiglinAi.setAngerTarget(world, piglin, (LivingEntity) optional.get());
        } else {
            PiglinAi.setAngerTarget(world, piglin, target);
        }
    ...
    private static void setAngerTargetIfCloserThanCurrent(ServerLevel world, AbstractPiglin piglin, LivingEntity target) {
        ...
        if (!optional.isPresent() || optional.get() != entityliving1) {
            PiglinAi.setAngerTarget(world, piglin, entityliving1);
        }
    }
