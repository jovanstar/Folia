...
public interface LevelReader extends ca.spottedleaf.moonrise.patches.chunk_system.level.ChunkSystemLevelReader, BlockAndTintGetter, CollisionGetter, SignalGetter, BiomeManager.NoiseBiomeSource { // Paper - rewrite chunk system
    ...
    default int getMaxLocalRawBrightness(BlockPos pos, int ambientDarkness) {
        return pos.getX() >= -30000000 && pos.getZ() >= -30000000 && pos.getX() < 30000000 && pos.getZ() < 30000000
            ? this.getRawBrightness(pos, ambientDarkness)
            : 15;
    }
    ...
    default boolean hasChunksAt(int minX, int minY, int minZ, int maxX, int maxY, int maxZ) {
        return maxY >= this.getMinY() && minY <= this.getMaxY() && this.hasChunksAt(minX, minZ, maxX, maxZ);
    }
    ...
    default <T> HolderLookup<T> holderLookup(ResourceKey<? extends Registry<? extends T>> registryRef) {
        Registry<T> registry = this.registryAccess().lookupOrThrow(registryRef);
