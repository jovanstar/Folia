...
public interface CollisionGetter extends BlockGetter {
    ...
    default boolean noCollision(@Nullable Entity entity, AABB box) {
        return this.noCollision(entity, box, false);
    }
    ...
    default Iterable<VoxelShape> getBlockCollisions(@Nullable Entity entity, AABB box) {
        return () -> new BlockCollisions<>(this, entity, box, false, (pos, voxelShape) -> voxelShape);
    }

    default Iterable<VoxelShape> getBlockAndLiquidCollisions(@Nullable Entity entity, AABB box) {
        return () -> new BlockCollisions<>(this, CollisionContext.of(entity, true), box, false, (mutableBlockPos, voxelShape) -> voxelShape);
    }
    ...
    default BlockHitResult clipIncludingBorder(ClipContext context) {
        BlockHitResult blockHitResult = this.clip(context);
        WorldBorder worldBorder = this.getWorldBorder();
        if (worldBorder.isWithinBounds(context.getFrom()) && !worldBorder.isWithinBounds(blockHitResult.getLocation())) {
            Vec3 vec3 = blockHitResult.getLocation().subtract(context.getFrom());
    ...
    default boolean collidesWithSuffocatingBlock(@Nullable Entity entity, AABB box) {
        BlockCollisions<VoxelShape> blockCollisions = new BlockCollisions<>(this, entity, box, true, (pos, voxelShape) -> voxelShape);
    ...
    default Optional<Vec3> findFreePosition(@Nullable Entity entity, VoxelShape shape, Vec3 target, double x, double y, double z) {
        if (shape.isEmpty()) {
            ...
            VoxelShape voxelShape = StreamSupport.stream(this.getBlockCollisions(entity, aABB).spliterator(), false)
                .filter(collision -> this.getWorldBorder() == null || this.getWorldBorder().isWithinBounds(collision.bounds()))
                .flatMap(collision -> collision.toAabbs().stream())
