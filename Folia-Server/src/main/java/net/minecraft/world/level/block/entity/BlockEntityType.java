...
public class BlockEntityType<T extends BlockEntity> {
    ...
    @Nullable
    public static ResourceLocation getKey(BlockEntityType<?> type) {
        return BuiltInRegistries.BLOCK_ENTITY_TYPE.getKey(type);
    }

    private static <T extends BlockEntity> BlockEntityType<T> register(String id, BlockEntityType.BlockEntitySupplier<? extends T> factory, Block... blocks) {
        if (blocks.length == 0) {
    ...
    public boolean onlyOpCanSetNbt() {
        return BlockEntityType.OP_ONLY_CUSTOM_DATA.contains(this);
    }
