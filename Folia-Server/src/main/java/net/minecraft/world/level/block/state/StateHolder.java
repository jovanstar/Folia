...
public abstract class StateHolder<O, S> implements ca.spottedleaf.moonrise.patches.blockstate_propertyaccess.PropertyAccessStateHolder { // Paper - optimise blockstate property access
    ...
    protected static <T> T findNextInCollection(List<T> values, T value) {
    ...
    public <T extends Comparable<T>> T getValueOrElse(Property<T> property, T fallback) {
        return Objects.requireNonNullElse(this.getNullableValue(property), fallback);
    }
    ...
    private <T extends Comparable<T>, V extends T> S setValueInternal(Property<T> property, V newValue, Comparable<?> oldValue) {
        if (oldValue.equals(newValue)) {
            ...
            int i = property.getInternalIndex((T)newValue);
            if (i < 0) {
    ...
    protected static <O, S extends StateHolder<O, S>> Codec<S> codec(Codec<O> codec, Function<O, S> ownerToStateFunction) {
        return codec.dispatch(
            ...
            state -> state.owner,
            owner -> {
                S stateHolder = ownerToStateFunction.apply((O)owner);
                return stateHolder.getValues().isEmpty()
                    ? MapCodec.unit(stateHolder)
                    : stateHolder.propertiesCodec.codec().lenientOptionalFieldOf("Properties").xmap(state -> state.orElse(stateHolder), Optional::of);
            }
        );
    }
}
