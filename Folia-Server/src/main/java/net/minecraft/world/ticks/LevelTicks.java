...
public class LevelTicks<T> implements LevelTickAccess<T> {
    ...
    public LevelTicks(LongPredicate tickingFutureReadyPredicate, net.minecraft.server.level.ServerLevel world, boolean isBlock) { this.world = world; this.isBlock = isBlock; // Folia - add world and isBlock
    ...
    public void schedule(ScheduledTick<T> orderedTick) {
        ca.spottedleaf.moonrise.common.util.TickThread.ensureTickThread(this.world, orderedTick.pos(), "Cannot schedule tick for another region!"); // Folia - region threading
        long l = ChunkPos.asLong(orderedTick.pos());
        LevelChunkTicks<T> levelChunkTicks = this.allContainers.get(l);
        if (levelChunkTicks == null) {
            Util.logAndPauseIfInIde("Trying to schedule tick in not loaded position " + orderedTick.pos());
        } else {
    ...
    public void copyAreaFrom(LevelTicks<T> scheduler, BoundingBox box, Vec3i offset) {
        ...
        scheduler.forContainersInArea(box, (chunkPos, chunkTickScheduler) -> chunkTickScheduler.getAll().filter(predicate).forEach(list::add));
